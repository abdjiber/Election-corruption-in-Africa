...
def visualize(list_cols_values, labels, figName, direction='H', saveFig=True):
    ...
    for i, col_values in enumerate(list_cols_values):
        ...
        if direction == 'H':
            count_col_values.plot(kind='barh', ax=ax, label=labels[i],
                                  color=constants.COLORS[i]
                                  )
        else:
            count_col_values.plot(kind='bar', ax=ax, label=labels[i],
                                  color=constants.COLORS[i]
                                  )
...
def plot_choropleth(df, z, layout_title, colorscale,
                    colorbar_title, tickvals, ticktext):
...
def plot_pca(data, labels, explained_variance,
             components, columns, figName,
    ...
    for i, lab in enumerate(labels.unique()):
        ax1.scatter(data[labels == lab, 0], data[labels == lab, 1],
                    label=lab, color=colors[i], s=60
                    )
...
def plot_2D(data, labels, components, columns,
            title, saveFig=True, legend=True):
    ...
    for lab in np.unique(labels):
        ...
        ax1.text(x.mean(), y.mean(), f'C{lab + 1}',
