...
def plot_feature_importances(df, target, kmeans_labels):
    fig = plt.figure(figsize=(12,6))
    fig.suptitle('Clusters comparision based on feature importances', **FIGURE_SUPTITLE_PARAMS)
    for i, u in enumerate(np.unique(kmeans_labels)):
        mask = kmeans_labels == u
        df_cluster = df.loc[mask]
        target_ = target[mask]
        f_imp = get_importante_variables(df_cluster, target_)
        ax = fig.add_subplot(2, 2, i + 1)
...
plot_feature_importances(df, target_2_classes, pred_labs)
...
def scores(y_val, preds, preds_prob):
    acc = np.round(accuracy_score(y_val, preds), 2)
    recall = np.round(recall_score(y_val, preds, average=None), 2)
    auc = np.round(roc_auc_score(y_val, preds_prob), 2)
    print('Accuracy:', acc)
    print('Recall:', recall)
    print('AUC:', auc)
...
class NeuralNets:
    ...
    def supervised_learning(self):
        ...
        acc, recall, auc = scores(self.y_val, self.preds, self.preds_prob)
...
def CV(model, train, target, n_splits=5):
    ...
    #pred = pd.DataFrame(np.zeros((te.shape[0], n_splits)), index=te.index)
    time_start = time.time()
    kf = KFold(n_splits=n_splits, random_state=4)
    for i, (tr_idx, te_idx) in enumerate(kf.split(train, target)):
        ...
        print('Fold {0} auc {1}'.format(i + 1, roc_auc_score(y_te, tmp)))
        oof.iloc[te_idx] = tmp
    time_end = time.time()
...
model, oof,  = CV(clf_lgb, df_targ_encoded, target_2_classes)


# In[127]:


scores(target_2_classes,  np.where(oof < 0.5, 0, 1), oof)
